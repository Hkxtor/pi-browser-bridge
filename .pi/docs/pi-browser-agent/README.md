...
Pi 浏览器 Agent 扩展。**当前默认状态**：启动本机 Relay 服务（`ws://127.0.0.1:16789/extension/v2`），等待真实 Chrome 扩展主动连入。工具、权限、审计、错误与取消语义已在 Mock 与 fake-extension 两层验证；**真实浏览器联动需在 dev 模式装个 Chrome 扩展现场验证**。
...
`controller_transport_unconfigured`（未配置）、`permission_denied`、`stale_element_ref`、`tab_not_found`、`cancelled`、`timeout`、`invalid_transport_result`、`internal_error` 等；每个错误带 `retryable` 与恢复建议。
